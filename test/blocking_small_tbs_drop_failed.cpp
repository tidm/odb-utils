...
void alter_user(){
        try{
        std::cerr << "\n****************\nALTERING USER" << std::endl;
        odb::transaction t0 (db_sys->begin ());
        odb::transaction::current (t0);
        db_sys->execute(std::string("alter user ") + user + " quota unlimited on USERS");
        std::cerr << "\n****************\nALTERED" << std::endl;
        t0.commit();
...
int main() {
    ...
    finished = false;
    oi_database* db;
    db_sys  = new oi_database("SYSTEM", "oiadmin1391", "10.0.0.166:1521/oi");
    srand(time(NULL));
    user = std::string( "OI_TMP" + std::to_string(std::abs(rand()%100000)));
    odb::transaction t0 (db_sys->begin ());
    db_sys->execute(std::string("create user ") + user + " identified by 1233 default tablespace USERS quota 500K on USERS");
    std::cerr << "user " << user << " created" << std::endl;
    db_sys->execute("grant connect,create table to " + user );
    std::cerr << "permission to user " << user << " granted " << std::endl;
    t0.commit();
    ...
    db_sys->execute(std::string("drop user ") + user + " cascade");
    t0.commit();
    if(cnt + failed != trans_count) {
        std::cerr << "trans_count: " << trans_count << " table includes: " << cnt << " failed:" << failed << std::endl;
        exit(-1);
    }
    else {
        std::cerr << "SUCCEEDED !trans_count: " << trans_count << " table includes: " << cnt << " failed:" << failed<< std::endl;
    }
    return 0;
}
