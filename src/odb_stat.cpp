...
oi::odb_stat::odb_stat(const stat_info& st):
    stat_info(st),
    _is_active {false},
    _que_len{0},
    _failure_count{0},
...
oi::odb_stat oi::odb_stat::operator+(const odb_stat& od) {
...
int oi::odb_stat::get_que_len() const {
    return _que_len;
}

int oi::odb_stat::get_fail_count() const {
    return _failure_count;
}

int oi::odb_stat::get_recover_count() const {
    return _recover_count;
}

void oi::odb_stat::set_que_len(size_t len) {
    _que_len = static_cast<int>(len);
}

void oi::odb_stat::update(uint64_t val, oi::execution_state st) {
    if(_is_active) {
        switch(st) {
